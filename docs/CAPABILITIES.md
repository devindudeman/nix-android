...
| Writable permission-policy flags | per-package `dumpsys package` runtime-permission rows | `pm set-permission-flags/clear-permission-flags --user 0` | reconciled exact state for the four bench-verified writable flags; `review-required` is rewritten by PermissionController and not offered | emits an exact list, including empty, for rows whose grant state the declaration reproduces; Android-owned flags remain snapshot evidence |
